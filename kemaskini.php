    ...
    <div class="card-body">
            ...
            if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_dir_file)) {
              ...
              while ($aF = zip_read($zipHandle) )
              {
                ...
                if ( !is_dir(BASE_URI.'/'.$thisFileName) ) {
                  ...
                  if ( $thisFileName == 'kemaskini.php' )
                  {
                    $upgradeExec = fopen ('kemaskini.php','w');
                    fwrite($upgradeExec, $contents);
                    fclose($upgradeExec);
                    include ('kemaskini.php');
                    unlink('kemaskini.php');
                    echo' EXECUTED</li>';
                  }
                  else
                  {
      ...
      <form method="post" enctype="multipart/form-data">
        <div class="group-set">Upload file (zip)
          ...
          <button type="submit" name="mybutton" value="versi" class="btn btn-success">Kemaskini Versi</button>
        </div>
      </form>
    </div>
  </div>
